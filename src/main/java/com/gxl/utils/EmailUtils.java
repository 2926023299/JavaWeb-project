...
public class EmailUtils {
	public static void sendEmail(User user){
		//发送方
		String myAccount = "2926023299@qq.com";
		//授权码
		String myPass = "kjdigmxgtbkodehd";
		//发件人 邮箱的 SMTP 服务器地址
	...
	private static MimeMessage createMsg(Session session, String myAccount, User user) {
		...
		try {
			//3.1发件人 from
			message.setFrom(new InternetAddress(myAccount, "华为", "utf-8"));
			...
			//3.3生成邮件主题
			message.setSubject("华为商城账号激活邮件","utf-8");
			String ip = Inet4Address.getLocalHost().getHostAddress();
			String url = "http://"+ip+":8080/UserServlet?method=activate&code="+ Base64Utils.encode(user.getCode());
			//设置邮件正文 setContent 可以使用html标签
			message.setContent(user.getUsername()+",你好<br>欢迎注册华为商城! 请点击链接进行激活:<a href='"+url+"'>"+url+"</a>","text/html;charset=utf-8");
			//设置邮件的发送时间 是立即发送

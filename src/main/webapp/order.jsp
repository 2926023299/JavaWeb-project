...
<head>
	...
	<script type="text/javascript">
        function createOrder(uid, sum) {
            location.href = "OrderServlet?method=create&uid=" + uid + "&sum=" + sum + "&aid=" + $("#address").val();
        }
	</script>
</head>
				...
				<tr>
					<td colspan="5">
						...
						<c:if test="${empty addList}">
							<a href="AddressServlet?method=show">添加收货地址</a>
						</c:if>
					</td>
				</tr>
			</table>
	...
	<div class="row pull-right" style="margin-right: 40px;">
		<div style="margin-bottom: 20px;">
			<button id="btn_add" class="btn  btn-danger btn-lg" type="button"
					onclick="createOrder(${loginUser.id}, ${sum})">提交订单
			</button>
		</div>
	</div>
</div>
